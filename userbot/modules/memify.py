...
async def memify(event):
    ...
    if not reply_msg:
        return await event.edit("**Ответьте на сообщение, содержущее медиа!**")

    if not reply_msg.media:
        return await event.edit("**Ответьте на иззображение/стикер/gif/видео!**")
    ...
    if input_file.endswith(".tgs"):
        await event.edit("**Распаковываю первое изображение...**")
        ...
        if not os.path.lexists(converted_file):
            return await event.edit("**Не могу разобрать анимированный стикер.**")
        input_file = converted_file

    elif input_file.endswith(".mp4"):
        await event.edit("**Получаю первый кадр...**")
        converted_file = os.path.join(TEMP_DOWNLOAD_DIRECTORY, "meme.png")
        await take_screen_shot(input_file, 0, converted_file)
        os.remove(input_file)
        if not os.path.lexists(converted_file):
            return await event.edit("**Не могу обработать это видео.**")
        input_file = converted_file

    await event.edit("**Добавляю текст...**")
    try:
        final_image = await add_text_img(input_file, input_str)
    except Exception as e:
        return await event.edit(f"**Произошла ошибка:**\n`{e}`")
...
CMD_HELP.update(
    {
        ...
        "\nДает текст на гифку/видео/стикер/фотку."
    }
)
